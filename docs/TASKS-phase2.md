...
**Phase 2 closed 2026-08-22.** All tasks were built, reviewed, and used on erebor. Real history verified both exit checks: “what do you know about X” and “what did we say about X.” On its first day, consolidation extracted records, tracing recorded the metrics, weekly review collected the first human verdicts, and `memory-replay` became the regression check for prompt changes. This is a historical record; `TASKS.md` is the live list.
...
Tasks are ordered by dependency; tasks with the same number could run in parallel. The phase had two parts: the **agent substrate** (1–4, adding tool support throughout the stack) and **memory** (5–8). Sections 5 and 6 each delivered value without waiting for 7.

Decisions made before planning, 2026-08-14:

- **Consolidation trigger: idle timeout.** A session untouched for a configurable window consolidates. This was a v1 default to tune from traces.
...
- **Routing comes later.** The eventual goal is a router that chooses among local and OpenRouter models. It needs Phase 2 usage data. This phase assumed only one provider.
- **Reasoning streams but is not stored.** The TUI shows it while the model thinks, ending Phase 1's silent pause. It is scratch work, not part of the durable conversation. `CompletionDelta` has a reasoning variant, the wire protocol carries it, and the TUI renders it dim. A reserved proto field leaves room to store it later without migrating old data.
...
**Two surprises.** This build did not require `--jinja`: tool calls and `reasoning_content` were unchanged without it, which matches how `arcd` already ran the sidecar. Second, the parser ignored the model's reasoning. It read `content` and discarded unknown fields, so hundreds of reasoning deltas never reached the TUI. Replaying the daemon's payload for “Say hello to arc in five words” produced 774 reasoning frames and 126–541 reasoning tokens for every trivial prompt. `openai_stream.sse` was not representative because it had seven completion tokens and no reasoning. Task 3.2 needed to choose whether to drop, stream, or store reasoning.
...
| 5.1 | Projection: messages shape extended for structured turns per the 2.1 sketch, FTS5 index over `content`, replay tests. Fold in the reserved `partial` column (`HistoryMessage` field 3) while the schema is open. Provider transcript only; the display half of §3.1's promise is 5.1b | claude | done |
| 5.1b | Reopened-session display: `SessionHistory` grows tool rows on the wire (additive), TUI renders past tool activity dim like live turns (decided 2026-08-21: split from 5.1 to keep diffs reviewable) | claude | done |
...
| 6.3 | `memory_read` / `memory_search` / `memory_write` / `memory_supersede` tools; writes emit events, never touch projection state directly (invariant 2) | claude | done |
...
Every task done; both exit criteria verified live on real history (2026-08-21). The first human review verdicts landed the same day: name accepted, storytelling deleted. The carried-forward list below moved to the live `TASKS.md` at the freeze; kept here as the record of what this phase left open:

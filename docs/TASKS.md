...
**Goal:** develop directly in bound code sessions, with one runner, mid-turn messages, visible tool results, and event-based compaction. Chat remains for talk, recall, and away-from-keyboard dispatch.

**Exit:** a week of development in `:code`, with compaction on real work and no visible context loss.
...
- Confirm deployment of the separate code role and paginated archive reads; the last recorded checks covered code and tests only.
...
- **Model-side image reading:** likely extend `read` with grant checks and image-bearing provider results. Attachments are separate; Bash/base64 is not vision input.

...
| 3.7 | **Gemini on the native API, carrying grounding.** Not a flag: `contents`/`parts`, camelCase, a `thoughtSignature` per part, and `toolCall`/`toolResponse` parts for server-side tools that the transcript has no representation for and that must round-trip like 2.3's blob. It buys Search and Maps grounding, which is what 4.1's `web` source resolves to for a Gemini role. Two things gate it. The terms require rendering the `searchEntryPoint` HTML snippet for Search and unmodified "Google Maps" source links immediately after the content they support — real TUI work, and unanswerable over audio, so Phase 4 needs a position on it before it needs anything else here. And grounding bills per search query the model chooses to run, not per prompt: 5,000 free a month across Gemini 3.x, then $14 per 1,000, and one prompt fired two to three queries in every probe. Nothing here is on the phase's exit criterion, so it waits | — | todo |

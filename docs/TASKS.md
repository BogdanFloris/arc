...
| 1.2 | The expert role — counsel, carried from Phase 3 at Bogdan's call: spike 1.2's two verified read-only templates (`claude -p` and `codex exec`, argv preserved in TASKS-phase3.md section 4) become a real part of the harness. DESIGN's role table already names counsel as the fourth role: plans, reviews, and unsticking, called a few times per job, selected for capability. To decide before building: whether it is a registry tool, a `[roles.counsel]` config entry resolved to an argv template rather than a provider, or both; which sessions hold it (an executor mid-job and the concierge both have claims); and how its output lands in the transcript. Read-only is already enforced on both CLIs; prefer `claude -p` when a diff may hold secrets, per the spike | — | todo |

...
- **Model routing:** select a configured model preset when creating a session or dispatching a job, based on task requirements, cost, and urgency. Keep the selected model pinned for that session; per-step switching is not proposed. Compare against static role defaults, including routing overhead and total task cost, not just per-call price. DESIGN §6.1 chooses static roles, so adopting runtime routing needs a design amendment.
...
- **Boundary:** projects now provide context, not filesystem containment. File tools and Bash run as the daemon user. A future hard boundary would need deterministic permissions or a sandbox; a classifier cannot grant itself authority or replace one.
...
Local execution is a privacy and availability requirement for this proposal, not evidence of decision correctness. Benchmark latency and resource contention on Erebor. DESIGN §4.3 and invariant 8 use tools with a scrubbed environment, not model approval or mid-turn prompts; adopting auto mode requires an explicit design change.

...
**Review verdicts** (amended 2026-08-22, at 8.1). The weekly review's three verdicts are ordinary events, `Event.source = USER`: *fix* is a `MemoryRecordSuperseded` (the corrected record replaces the wrong one), *delete* is a `MemoryRecordDeleted`, and *accept* is a new skip-safe kind, `MemoryRecordReviewed { record_id }` — durable because reviews are the ground truth this section rests on: without an accept event, "human-confirmed" is not a fact the log can answer, and the precision/recall sampling below has no labels. The projection stamps each record with `changed_at` (last create/update/supersede) and `reviewed_at`; the review list is "records changed in the window and not reviewed since their last change", so an accepted record leaves the queue and a later change re-enters it. Fixing happens through conversation, not a TUI editor: the review pane prefills the chat input with a supersede instruction the user edits and sends, and the model writes through the ordinary tools — the review UI never mutates memory directly.

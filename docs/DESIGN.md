...
- **The selected archivist model writes the summary.** Compaction is a separate call, not a change to the session's pinned model. The span carries the compaction model and usage. Validate before appending; if the first draft fails validation, offer that draft and the reason to the archivist for one bounded repair, without resending the history. A second failure appends nothing and surfaces an error to the client; never fall back to the session's model.

...
| # | Task | Assignee | Status |
|---|------|----------|--------|
| 6.1 | Connect + session picker + send message + streaming render (absorbs old 6.2: "message pane, input line, session picker, nothing else") | claude | in progress |
...
- Look: pure ASCII — no box-drawing, no nerd-font glyphs, no borders; structure from whitespace and `--` rules. Terminal ANSI-16 colors only (never RGB), so the user's gruvbox terminal theme maps through; semantic color roles in one small theme module. Orange is the single accent. Small lowercase wordmark on the empty state only.

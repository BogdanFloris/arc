...
| 2.3 | Segment rollover by size + segment file naming. Add a shared `MAX_RECORD_LEN` sanity cap to `log::format` (writer enforces, reader rejects) | claude | todo |

## 3. SQLite projection (`arc-core`)

| # | Task | Assignee | Status |
|---|------|----------|--------|
| 3.1 | Schema: `sessions` + `messages` tables, projection struct over rusqlite | — | todo |
| 3.2 | Replay: log in → state out, resumable from last projected seq; deterministic replay test | — | todo |

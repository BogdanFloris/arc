# CLAUDE.md — ARC (Autonomous Robotic Core)

Personal AI assistant harness: always-on Rust daemon, thin clients, event-sourced memory, multi-provider LLM support. **`DESIGN.md` is the architectural authority.** If a task conflicts with it, stop and flag the conflict — amend DESIGN.md first, then implement. Do not silently diverge.
...
Phase 1 — walking skeleton (see DESIGN.md §11). Do not implement features from later phases, even if convenient. When a Phase 1 task tempts you toward Phase 2+ scope, build the seam, not the feature.
...
Prefer: smaller diff, seam over feature, DESIGN.md over cleverness, asking over assuming. Open questions in DESIGN.md §12 are deliberately unresolved — do not resolve them in code.
